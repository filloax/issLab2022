...
public class RadarSysSprint2ControllerOnRaspMain implements IApplication{
	...
	protected void configure() {		
		...
		String proxyName = "radarPxy";
		radar = new RadarGuiProxyAsClient(
				proxyName,
				RadarSystemConfig.hostAddr,
				Integer.toString(RadarSystemConfig.serverPort),
				ProtocolType.tcp
		);
		//Controller
	...
	protected void execute() {
		...
		int d = radar.getCurDistance().getVal();
		ColorsOut.outappl("CURRENT DISTANCE answer=" + d,ColorsOut.MAGENTA );
		controller.start(endFun, 10000);
	}
